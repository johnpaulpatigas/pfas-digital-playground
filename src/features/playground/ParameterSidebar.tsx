      ...
      <div className="flex items-center justify-between gap-2 px-1">
        {!isSimpleMode ? (
          <div className="flex items-center gap-1 overflow-x-auto py-1">
            {DEMOGRAPHIC_PRESETS.map((preset) => (
              <button
                ...
                className={`px-2 py-1 rounded text-[11px] font-mono whitespace-nowrap transition-colors cursor-pointer border ${
          ...
          <span className="text-[11px] font-mono text-slate-500 font-medium">
            3-Input Profile &bull; Automated Toxicokinetics
          </span>
        )}
        ...
        <button
          ...
          className="p-1 text-slate-400 hover:text-slate-700 transition-colors ml-auto cursor-pointer"
      ...
      {isSimpleMode ? (
        <div className="space-y-3">
          ...
          <div className="card-panel p-3 rounded-xl space-y-2 hover:border-slate-300 transition-colors">
            ...
            <div className="flex items-center justify-between gap-1 pt-0.5">
              {[1.0, 1.5, 2.0, 3.0].map((presetVal) => (
                <button
                  key={presetVal}
                  type="button"
                  onClick={() => updateSimpleProfile({ waterConsumption: presetVal })}
                  className={`px-2 py-0.5 rounded text-[10px] font-mono transition-all cursor-pointer border ${
                    Math.abs(simpleProfile.waterConsumption - presetVal) < 0.05
                      ? 'bg-teal-600 text-white border-teal-600 font-bold shadow-2xs'
                      : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
                  }`}
                >
                  {presetVal.toFixed(1)} L
                </button>
              ))}
            </div>
          </div>
